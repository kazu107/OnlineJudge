...
async function handleStandardEvaluation(req, res, flush, problemId, language, code, meta, tmpDir, solutionPath) {
    ...
    // Send preliminary test suite information
    const testSuiteInfoStandard = [];
    for (const category of categories) {
        for (const testCase of category.test_cases) {
            const testCaseName = path.basename(testCase.input);
            // Ensure ID is unique and suitable for use in HTML/JS contexts
            const safeCategoryName = category.category_name.replace(/\s+/g, '_').replace(/[^a-zA-Z0-9_]/g, '');
            const safeTestCaseName = testCaseName.replace(/[^a-zA-Z0-9_.-]/g, '');
            const id = `${safeCategoryName}_${safeTestCaseName}`;
            testSuiteInfoStandard.push({
                id: id,
    ...
    sendSse(res, { type: 'test_suite_info', test_cases: testSuiteInfoStandard }, flush);
...
async function handleCustomEvaluation(req, res, flush, problemId, language, code, meta, tmpDir) {
    let totalPointsEarned = 0;
    // max_total_points is a top-level field defined in meta.json
    const maxTotalPoints = meta.max_total_points; 
    ...
    if (!meta.test_case_categories || !Array.isArray(meta.test_case_categories)) {
        sendSse(res, { type: 'error', message: 'Invalid problem metadata: test_case_categories not found or not an array for custom evaluation.' }, flush);
        // Ensure tmpDir is cleaned up, though finally block in handler should also do it.
        // fs.rmSync(tmpDir, { recursive: true, force: true }); // Covered by main handler's finally
        res.end(); // End the response if meta is malformed for this path
        return;
    }

    // Send preliminary test suite information
    const testSuiteInfoCustom = [];
    for (const category of meta.test_case_categories) {
        for (const testCase of category.test_cases) {
            const testCaseId = testCase.id || path.basename(testCase.data_file);
            const description = testCase.description || testCaseId; // Use description, fallback to id
            testSuiteInfoCustom.push({
                id: testCaseId, // Already unique
                category_name: category.category_name,
                name: description
            });
        }
    }
    sendSse(res, { type: 'test_suite_info', test_cases: testSuiteInfoCustom }, flush);
    ...
    for (const category of meta.test_case_categories) {
        ...
        try {
            ...
            // Read stdin from testCase.data_file for the user program
            let stdinContent = "";
            const dataFilePath = path.join(process.cwd(), testCase.data_file);

            if (fs.existsSync(dataFilePath)) {
                stdinContent = fs.readFileSync(dataFilePath, 'utf8');
            } else {
                testCaseStatus = "System Error";
                const errorMessage = `Test data file not found: ${testCase.data_file}`;
                sendSse(res, {
            ...
            const userCodeExecTimeoutMs = meta.time_limit_ms || 5000; // Ensure this aligns with what executor service expects
            const userCodeMemoryLimitKb = meta.memory_limit_kb || 256000; // Default if not specified

            const executePayload = {
                language,
                code,
                stdin: stdinContent,
                time_limit_ms: userCodeExecTimeoutMs,
                memory_limit_kb: userCodeMemoryLimitKb
            };
            ...
            const executeResponse = await fetch(executorExecuteUrl, {
                ...
                timeout: userCodeExecTimeoutMs + 1000 // Network timeout slightly larger
            });
        ...
        if (testCaseStatus === "User Code Executed") {
            ...
            // Use testCase.eval_aux_data_file for the evaluator's data (e.g., JSON with city coordinates, optimal_distance)
            const auxDataFilePath = testCase.eval_aux_data_file ? path.join(process.cwd(), testCase.eval_aux_data_file) : null;
            ...
            } else if (!auxDataFilePath || !fs.existsSync(auxDataFilePath)) { // Check if auxDataFilePath is valid and exists
                testCaseStatus = "System Error"; 
                message = `Auxiliary test data file ${testCase.eval_aux_data_file || 'not specified'} not found on host.`;
            } else {
                const evaluatePayload = {
                    evaluator_script_host_path: evaluatorScriptHostPath,
                    user_solution_host_path: userOutputFilePathOnHost,
                    test_data_host_path: auxDataFilePath, // Changed to auxDataFilePath
...
export default async function handler(req, res) {
    ...
    try {
        if (meta.evaluation_mode === "custom_evaluator") {
            // Check for test_case_categories for custom evaluation, as test_cases is deprecated
            if (!meta.custom_evaluator_options || !meta.custom_evaluator_options.evaluator_script || !meta.test_case_categories) {
                sendSse(res, { type: 'error', message: 'Invalid meta.json for custom_evaluator: missing custom_evaluator_options, evaluator_script, or test_case_categories array.' }, flush);
            } else {
